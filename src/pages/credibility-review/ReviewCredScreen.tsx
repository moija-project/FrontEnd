...
export default function ReviewCredScreen() {
  ...
  return (
    <CommonContainer>
      ...
      <ReviewContentContainer postId={state.postId} type={type} setSumScore={handleSumScore} peerId={peerId} />

      <TotalScoreWrapper>
        <CalculatedTotal>{totalScore === 0 ? ' ' : totalScore + ' 점'}</CalculatedTotal>
...
const SubmitButton = styled.button<{ pressable: boolean }>`
  ...
  background-color: ${({ pressable }) => (pressable ? 'var(--purple)' : 'var(--gray01)')};
  color: white;
  text-align: center;
  padding: 0.7rem;
  display: flex;
